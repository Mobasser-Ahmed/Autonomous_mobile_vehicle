...
def augmentImage(imgPath,steering):
    ...
    #15.06 commented because felt like not required
...
# imag = mpimg.imread('DataCollectedBFixed/IMG31/Image_164858404340338.jpg')
# imgRe = preProcess(imag)
# # mpimg.imsave('Result.jpg',imgRe)
# plt.imshow(imgRe)
# plt.show()
...
def dataGen(imagesPath, steeringList, batchSize, trainFlag):
    ...
    while True:
        ...
        steeringBatch = []
        i = 0
        for index in np.random.permutation(imagesPath.shape[0]):
            if trainFlag:
...
def getAllImg(imagesPath, steeringList):
    while True:
        imgBatch = []
        steeringBatch = []
        ...
        for i in range(len(imagesPath)):
            ...
            img = preProcess(img)
            imgBatch.append(img)
            steeringBatch.append(steeringList[i])

    yield (np.asarray(imgBatch), np.asarray(steeringBatch))


def mo_plot(y_train, num_bins, y_valid):
